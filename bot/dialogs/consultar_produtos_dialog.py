...
class ConsultarProdutoDialog(ComponentDialog):
    ...
    async def prompt_process_product_name_step(self, step_context: WaterfallStepContext):
        ...
        if produtos and len(produtos) > 0:
            ...
            # Mapear os campos da API para os campos esperados
            nome = produto.get("productName", "Nome não disponível")
            categoria = produto.get("productCategory", "Categoria não disponível")
            preco = produto.get("price", 0)
            descricao = produto.get("productDescription", "Descrição não disponível")

            # Para as imagens, pegar a primeira se houver uma lista
            url_imagem = None
            if produto.get("imageUrl"):
                if isinstance(produto["imageUrl"], list) and len(produto["imageUrl"]) > 0:
                    url_imagem = produto["imageUrl"][0]
                elif isinstance(produto["imageUrl"], str):
                    url_imagem = produto["imageUrl"]
            ...
            card = CardFactory.hero_card(
                HeroCard(
                    ...
                    text=descricao,
                    images=[CardImage(url=url_imagem)] if url_imagem else [],
                    buttons=[
                        CardAction(
